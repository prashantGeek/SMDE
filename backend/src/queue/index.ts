...
export const documentWorker = new Worker("documentExtraction", async job => {
  ...
  try {
    ...
    await query(
      "UPDATE jobs SET status = 'FAILED', error_code = 'UNHANDLED_ERROR', error_message = $1, completed_at = CURRENT_TIMESTAMP WHERE id = $2",
      [uncaughtError.message || "An unknown error occurred during job processing.", jobId]
    );
    throw uncaughtError;
  }
